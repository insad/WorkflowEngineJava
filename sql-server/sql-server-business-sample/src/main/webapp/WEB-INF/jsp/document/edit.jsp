...
<%
    ...
    UUID currentUserId = CurrentUserSettings.getCurrentUser(request, response);
    Employee author = EmployeeHelper.getEmployee(currentUserId);
    ...
    if (model != null && model.getId() != null) {
        ...
        commands = DocumentHelper.getCommands(model.getId(), UUIDUtil.asString(currentUserId));
        ...
        <form action="/Document/Edit" id="form0" method="post">
            ...
            <table class="table">
                ...
                <tr>
                    ...
                    <td>
                        <input id="Author" name="Author" type="hidden" value="${model.author.id}">
                            ${model.author.name}
                    </td>
                </tr>
                <tr>
                    <td><label for="EmployeeController">Controller</label></td>
                    <td>
                        <select id="EmployeeController" name="EmployeeController" style="width:98%">
                            <option <c:if test="${model.employeeController == null}">selected
                            </c:if></option>
                            <c:forEach items="${employeeList}" var="item">
                                <option value="${item.id}"
                                        <c:if test="${model.employeeController.equals(item)}">selected</c:if>>
                                    Name: ${item.name}; StructDivision: ${item.structDivision.name},
